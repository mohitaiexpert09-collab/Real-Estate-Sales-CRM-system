...
export async function seedDatabase() {
  ...
  const agents = [];
  for (const name of AGENTS) agents.push(await prisma.agent.create({ data: { name } }));
  ...
  for (let i = 0; i < N; i++) {
    ...
    const budget = Math.round((p.base * (0.85 + ((i * 37) % 40) / 100)) / 100000) * 100000;
    ...
    const siteVisitAt = ["Site Visit", "Negotiation", "Booked"].includes(status) ? new Date(createdAt.getTime() + 2 * 86400000) : null;

    const lead = await prisma.lead.create({
      data: {
        name,
        phone: `+91 9${(800000000 + i * 137911).toString().slice(0, 9)}`,
        source, project: p.project, city: p.city, budget, status, score, tier, intent, timeline,
        aiSummary: summary, agentId: agent.id, firstResponseSecs: frs, siteVisitAt, createdAt,
      },
    });

    await prisma.message.create({ data: { leadId: lead.id, direction: "in", body: msg, createdAt } });
    await prisma.message.create({
      data: {
        leadId: lead.id, direction: "out", isAuto: true,
    ...
    if (["Qualified", "Site Visit", "Negotiation", "Booked"].includes(status)) {
      await prisma.message.create({ data: { leadId: lead.id, direction: "in", body: "Yes please share the details.", createdAt: new Date(createdAt.getTime() + 6 * 3600000) } });
    }
